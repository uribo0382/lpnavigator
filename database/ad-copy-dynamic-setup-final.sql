...
-- ad_copiesテーブルにsource列がない場合は追加（互換性のため）
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'ad_copies' 
    AND column_name = 'source'
  ) THEN
    ALTER TABLE ad_copies ADD COLUMN source VARCHAR(50);
    UPDATE ad_copies SET source = generated_by WHERE source IS NULL;
  END IF;
END $$;
...
-- 既存のad_copyタイプのフォーミュラを削除（重複を避けるため）
DELETE FROM formulas WHERE type = 'ad_copy';
...
-- 挿入されたフォーミュラの確認
SELECT COUNT(*) as formula_count FROM formulas WHERE type = 'ad_copy';

-- 挿入された基本情報の確認
SELECT COUNT(*) as basic_info_count FROM basic_infos;
...
-- 3. 広告文履歴ビュー
-- 
-- 広告文生成機能が正常に動作するようになります。
-- 現在はGemini 2.0 Flashのみが有効化されています。